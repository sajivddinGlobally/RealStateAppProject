...
class _ListingPageState extends ConsumerState<ListingPage> {
  ...
  final List<String> cityOptions = ["Jaipur", "Gwalior", "Ajmer", "Delhi", "Mumbai", "Bangalore"];
  late List<bool> selectedCities;
  final List<String> items = ["1 BHK", "2 BHK", "3 BHK", "4 BHK", "5 BHK", "6 BHK", "7 BHK", "8 BHK"];
  late List<bool> selected;
  final List<String> searchItem = ["Gwalior", "ajmer", "Jaipur", "Delhi", "6 BHK", "Delhi"];
  late List<bool> searchSelect;
  @override
  void initState() {
    super.initState();
    ...
    if (widget.initialData != null) {
      final city = widget.initialData!.city?.toLowerCase() ?? "";
      final idx = cityOptions.map((e) => e.toLowerCase()).toList().indexOf(city);
      if (idx != -1) {
  ...
  void _resetFilters() {
    setState(() {
      ...
      searchSelect.fillRange(0, searchSelect.length, false);
      currentPage = 1;
  ...
  void _applyFilters() {
    setState(() {
      ...
      body = PropertyListBodyModel(
        ...
        sortBy: 'createdAt',
        sortOrder: 'desc',
        // ← Add priceMin / priceMax / bedrooms / cities here later if you send to backend
      );
    });
  }
  @override
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          onChanged: (val) {
                            final max = double.tryParse(val.replaceAll(',', '')) ?? 10000000;
                            setState(() {
                              _priceRange = RangeValues(_priceRange.start, max.clamp(_priceRange.start, 10000000));
                            });
                          },
                        ),
                      ),
                  ...
                  Text("Locality", style: GoogleFonts.inter(fontSize: 16.sp, fontWeight: FontWeight.w500)),
                  ...
                  ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return CheckboxListTile(
                        value: searchSelect[index],
                        activeColor: const Color(0xffFF6A2A),
                        title: Text(searchItem[index], style: GoogleFonts.inter(fontSize: 14.sp)),
                        dense: true,
                        contentPadding: EdgeInsets.zero,
                        onChanged: (value) {
                          setState(() {
                            searchSelect[index] = value!;
                            final cityVal = searchItem[index].toLowerCase();
                            final idx = cityOptions.map((c) => c.toLowerCase()).toList().indexOf(cityVal);
                            if (idx != -1) selectedCities[idx] = value;
                          });
                        },
                      );
                    },
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text("Search", style: GoogleFonts.inter(fontSize: 16.sp, fontWeight: FontWeight.w600)),
            ...
            //     data: (res) {
            //
            //       widget.initialData.property->commercial,residential,
            //       widget.initialData.listingCategory=>buy,rent
            ...
            //               else if (bhkStr == opt) bhkMatch = true;
            //             }
            //           }
            //           if (!bhkMatch) return false;
            //         }
            //
            //         final city = prop.city?.toLowerCase() ?? '';
            //         if (selectedCities.any((sel) => sel)) {
            //           bool cityMatch = false;
            //           for (int i = 0; i < cityOptions.length; i++) {
            //             if (selectedCities[i] && city == cityOptions[i].toLowerCase()) {
            //               cityMatch = true;
            //               break;
            //             }
            //           }
            ...
            Padding(
              ...
              child: propertyAsync.when(
                ...
                data: (res) {
                  ...
                  final filteredList = allProperties.where((prop) {
                    // ────────────────────────────────────────────────
                    //  MOST IMPORTANT: Strict initial category filtering
                    // ────────────────────────────────────────────────
                    final propType = prop.property?.toLowerCase() ?? '';
                    final listCat  = prop.listingCategory?.toLowerCase() ?? '';

                    if (propType != widget.initialData.property!.toLowerCase()&&listCat != widget.initialData.listingCategory!.toLowerCase()) {
                      return false;
                    }
...
*/
/*  if (listCat != widget.initialData.listingCategory!.toLowerCase()) {
                      return false;
                    }*/ /*
*/
/*
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          "${int.tryParse(property.bedRoom ?? '0') ?? '?'} BHK ${property.propertyType ?? 'Property'}",
                          style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold),
                        ),
                        const SizedBox(height: 8),
                        Text(
                          "₹ ${_formatPrice(property.price)} • ${property.listingCategory?.toUpperCase() ?? ''}",
                          style: const TextStyle(fontSize: 20, color: Color(0xffFF6A2A)),
                        ),
                        const SizedBox(height: 8),
                        Text("${property.localityArea ?? ''}, ${property.city ?? ''}"),
                        const SizedBox(height: 16),
                        ...
                        ElevatedButton(
                          ...
                          child: const Text("Contact Owner", style: TextStyle(color: Colors.white, fontSize: 16)),
                        ),
                      ],
                    ),
                  ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: GoogleFonts.inter(color: const Color(0xffFF6A2A), fontWeight: FontWeight.bold, fontSize: 15.sp),
                  ),
                  SizedBox(height: 12.h),
                  Row(
                    children: [
                      Expanded(
                        child: OutlinedButton(
                          ...
                          onPressed: () {},
                          child: Text("View", style: GoogleFonts.inter(fontSize: 12.sp)),
                        ),
                      ),
                      SizedBox(width: 8.w),
                      Expanded(
                        child: ElevatedButton(
                          style: ElevatedButton.styleFrom(backgroundColor: const Color(0xffFF6A2A), padding: EdgeInsets.zero),
                          onPressed: () {},
                          child: Text("Contact", style: GoogleFonts.inter(color: Colors.white, fontSize: 12.sp)),
                        ),
                      ),
                    ],
                  ),
...
class ListingPage extends ConsumerStatefulWidget {
  final ListElement? initialData; // optional → property type & listing category ke liye
...
class _ListingPageState extends ConsumerState<ListingPage> {
  ...
  final List<String> searchItem = ["Jaipur", "Jodhpur", "Gwalior", "Ajmer", "Delhi"];
  ...
  void initState() {
    ...
    // Default city pre-select karo agar diya gaya ho


    _resetFilters();
    _applyFilters(); // pehli baar load karte waqt filters apply
  }
  ...
  void _resetFilters() {
    setState(() {
      ...
      selectedBHK.fillRange(0, selectedBHK.length, false);
      // selectedCities ko reset mat karo → default city rehne do
      selected.fillRange(0, selected.length, false);
      searchSelect.fillRange(0, searchSelect.length, false);
  ...
  void _applyFilters() {
    setState(() {
      ...
      body = PropertyListBodyModel(
        ...
        sortBy: 'createdAt',
        sortOrder: 'desc',
        // Agar backend in fields ko support karta hai to yahan add kar sakte ho
        // property: widget.initialData?.property,
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text("City / Location", style: GoogleFonts.inter(fontSize: 16.sp, fontWeight: FontWeight.w500)),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      icon: const Icon(Icons.search, color: Colors.white),
                      label: Text("Apply Filters", style: GoogleFonts.inter(
                          color: Colors.white,
            ...
            Padding(
              ...
              child: propertyAsync.when(
                ...
                data: (res) {
                  ...
                  final filteredList = allProperties.where((prop) {
                    // ────────────────────────────────────────────────
                    // SABSE PEHLE → Property type aur Listing category DONO match hone chahiye
                    // ────────────────────────────────────────────────
                    if (widget.initialData != null) {
                      final expectedProperty = widget.initialData!.property?.toLowerCase() ?? '';
                      final expectedCategory = widget.initialData!.listingCategory?.toLowerCase() ?? '';

                      final actualProperty = prop.property?.toLowerCase() ?? '';
                      final actualCategory = prop.listingCategory?.toLowerCase() ?? '';

                      // Agar KUCH BHI match nahi karta → is property ko dikhao MAT
                      if (actualProperty != expectedProperty || actualCategory != expectedCategory) {
                        return false;
                      }
                    }
                    ...
                    if (selectedCities.any((sel) => sel)) {
                      bool cityMatch = false;
                      for (int i = 0; i < cityOptions.length; i++) {
                        if (selectedCities[i] && propCity == cityOptions[i].toLowerCase()) {
                          cityMatch = true;
                          break;
                        }
                      }
...
//
//       debugPrint("ListingPage: Hive se saved city mila → $saved");
//
...
//       }
//     } catch (e) {
//       debugPrint("Hive load error in ListingPage: $e");
//     }
//   }
//
//   void _resetFilters() {
...
// New: Search provider (you can also put this in a separate providers file)
    ...
    return Scaffold(
      ...
      body: cityAsync.when(
        ...
        data: (cityResponse) {
          ...
          //       .toList();
          //   selectedCities = List<bool>.filled(cityOptions.length, false);
          //   debugPrint("Cities loaded: ${cityOptions.join(', ')}");
          //   // Priority: Hive saved city
          //   if (_savedCityFromHive != null) {
          //     final normalized = _savedCityFromHive!.toLowerCase().trim();
          //     final idx = cityOptions
          //         .map((c) => c.toLowerCase().trim())
          //         .toList()
          //         .indexOf(normalized);
          ...
          //         .toList()
          //         .indexOf(incoming);
          //     if (idx != -1) selectedCities[idx] = true;
          //   }
          //   _citiesLoaded = true;
          // }
          // Priority 1 → HomeScreen selected city
          if (!_citiesLoaded && cityResponse?.data != null) {
            cityOptions = cityResponse.data!
                .map((d) => d.cityName ?? "")
                .where((name) => name.isNotEmpty)
